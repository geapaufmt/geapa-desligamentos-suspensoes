...
function off_processQueue() {
  ...
  try {
    ...
    rows.forEach((row, idx) => {
      ...
      // 2) Se deferido, enviar e-mail final ao solicitante (uma vez)
      if (approved === OFF_CFG.VALUES.DECISAO_DEFERIDO && sentFinal !== OFF_CFG.VALUES.YES) {
        off_sendFinalEmailToMember_({ tipo, nome, rga, email });

        sh.getRange(sheetRow, cSentFinal).setValue(OFF_CFG.VALUES.YES);
        if (cDeferDt) sh.getRange(sheetRow, cDeferDt).setValue(now);
        finalSentCount++;
      }
    });

    GEAPA_CORE.coreLogInfo(runId, 'off_processQueue: FIM OK', { notifiedCount, finalSentCount });

  } catch (e) {
    GEAPA_CORE.coreLogError(runId, 'off_processQueue: ERRO', { err: String(e), stack: e && e.stack });
    throw e;
  } finally {
    lock.releaseLock();
  }
